...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_CREATE:
        ...
        rect.left = 20 * cxChar;
        rect.top = 3 * cyChar;
        hwndList = CreateWindow(TEXT("listbox"), NULL, WS_CHILDWINDOW | WS_VISIBLE | LBS_STANDARD, cxChar, cyChar * 3, cxChar * 13 + GetSystemMetrics(SM_CXVSCROLL), 
    ...
    case WM_PAINT:
        ...
        ReadFile(hFile, buffer, MAXREAD, &numberOfBytesToRead, NULL);
        CloseHandle(hFile);
        ...
        DrawTextA(hdc, buffer, (int)numberOfBytesToRead, &rect, DTFLAGS);*/
        szTemp = buffer;
        
        //MultiByteToWideChar(CP_ACP, 0, reinterpret_cast<char*>(buffer), -1, wStr, numberOfBytesToRead);
        DrawText(hdc, wStr, (int)numberOfBytesToRead, &rect, DTFLAGS);
        //DrawText(hdc, reinterpret_cast<const wchar_t *>(buffer), (int)numberOfBytesToRead, &rect, DTFLAGS);
        DrawText(hdc, szTemp.GetString(), (int)numberOfBytesToRead, &rect, DTFLAGS);
        EndPaint(hWnd, &ps);
        break;
    case WM_DESTROY:
        PostQuitMessage(0);
